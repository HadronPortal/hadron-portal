...
const RelatorioPedidos = () => {
  ...
  const { data, isLoading, isFetching, error: queryError } = useApiFetch<any>({
    queryKey: ['analytics-orders', String(page), String(rowsPerPage), repParam || 'all', dateIniParam, dateEndParam, searchQuery.trim(), String(filterNonce)],
  ...
  const orders: OrderAnalytics[] = data?.orders || data?.products || [];
  const periods: Period[] = (data?.periods || []).filter((p: Period) => p.chave !== 'TOTAL');
  const totalRecords: number = data?.total_records || 0;

  const tabFiltered = activeTab === 'todos' ? orders : orders.filter((o) => o.tipo_op_banco === activeTab);
  const filtered = useMemo(() => {
    if (!searchInput.trim()) return tabFiltered;
    const q = searchInput.toLowerCase();
    return tabFiltered.filter(o => (o.cliente || '').toLowerCase().includes(q));
  }, [tabFiltered, searchInput]);

  const grandTotal = filtered.reduce((acc, o) => ({
    valor: acc.valor + (o.totais?.valor || 0),
    qtde: acc.qtde + (o.totais?.qtde || 0),
  }), { valor: 0, qtde: 0 });
    ...
    <>
      ...
      {error ? (
        ...
        <div className="px-2 sm:px-6 py-4">
          <SkeletonTable columns={4} rows={10} headers={['PEDIDO / CLIENTE', 'MÊS 1', 'MÊS 2', 'TOTAL']} />
        </div>
      ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-t border-b border-border">
                  <th className="px-5 py-3 text-left text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Pedido / Cliente</th>
                  {periods.map((p) => (
              ...
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  filtered.map((ord, idx) => (
                    <tr key={ord.codpedido || idx} className={`border-b border-border hover:bg-accent/30 transition-colors ${idx % 2 === 1 ? 'bg-muted/30' : ''}`}>
                      <td className="px-5 py-3 text-sm">
                        <div>
                          <span className="font-medium text-foreground">{ord.cliente}</span>
                          <div className="text-xs text-muted-foreground">#{ord.codpedido} · {ord.cidade} - {ord.uf}</div>
                        </div>
                      </td>
                      {periods.map((p) => {
                        const m = ord.mensal?.[p.chave];
                        return (
                      ...
                      <td className="px-4 py-3 text-sm text-right">
                        <div className="font-semibold text-foreground">{formatCurrency(ord.totais?.valor || 0)}</div>
                        <div className="text-xs text-muted-foreground">{ord.totais?.qtde || 0} un</div>
                      </td>
                    </tr>
                  ))
                )}
