...
const RelatorioPedidos = ({ filters }: { filters: SharedFilterProps }) => {
  ...
  const { data, isLoading, isFetching, error: queryError } = useApiFetch<any>({
    queryKey: ['report-orders', String(page), String(rowsPerPage), repParam || 'all', codterParam || 'all', toApiDate(filters.selectedPeriod.startDate), toApiDate(filters.selectedPeriod.endDate), filters.searchQuery.trim(), String(filters.filterNonce)],
