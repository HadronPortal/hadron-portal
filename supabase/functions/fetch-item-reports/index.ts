...
serve(async (req) => {
  ...
  try {
    ...
    const res = await fetch('https://dev.hadronweb.com.br/DEV/app/Pages/apiItemReports', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({ codpro: parseInt(codpro), orc_codrep: [3] }),
    });
    ...
    if (!res.ok) {
      cachedToken = null;
      throw new Error(`ItemReports fetch failed [${res.status}]: ${responseText.substring(0, 500)}`);
    }
